...
export const useQuizStore = create<QuizState>((set) => ({
    currentQuestion: 0,
    answers: Array(3).fill(null), // Fixed: Matches number of questions (3)
    score: 0,
    showScore: false,
    questions: [
        {
    ...
    nextQuestion: () =>
        set((state) => {
            ...
            if (isLastQuestion) {
                ...
                let score = 0;
                state.questions.forEach((ques, index) => {
                    if (state.answers[index] === ques.correctAnswer) {
                        score++;
                    }
                });
    ...
    resetQuiz: () =>
        set(() => ({
            currentQuestion: 0,
            answer: Array(3).fill(null), // Fixed: Matches number of questions
            score: 0,
            showScore: false
        }))
}));